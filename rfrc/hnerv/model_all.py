    ...
    def __init__(self, args):
        if os.path.isfile(args.data_path):
            ...
            self.video = [
                ...
                for x in sorted(os.listdir(args.data_path))
            ]
...
def RandomMask(height, width, points_num, scale=(0, 1)):
    polygon = [
        (x, y)
        for x, y in zip(
            np.random.randint(height * scale[0], height * scale[1], size=points_num),
    ...
    def forward(self, img):
        ...
        if "inpaint" in self.vid:
            ...
            for ctr_x, ctr_y in [
                ...
                inpaint_mask[
                    ctr_x - self.inpaint_size : ctr_x + self.inpaint_size,
                    ctr_y - self.inpaint_size : ctr_y + self.inpaint_size,
                ] = 0
