...
def adjust_lr(optimizer, cur_epoch, args):
    ...
    if "hybrid" in args.lr_type:
        up_ratio, up_pow, down_pow, min_lr, final_lr = [
            float(x) for x in args.lr_type.split("_")[1:]
        ]
        if cur_epoch < up_ratio:
            lr_mult = min_lr + (1.0 - min_lr) * (cur_epoch / up_ratio) ** up_pow
        else:
            lr_mult = (
                1
                - (1 - final_lr)
                * ((cur_epoch - up_ratio) / (1.0 - up_ratio)) ** down_pow
            )
        ...
        if cur_epoch < up_ratio:
            ...
            lr_mult = 0.5 * (
                math.cos(math.pi * (cur_epoch - up_ratio) / (1 - up_ratio)) + 1.0
            )
    else:
...
def Diff_decompose(img_diff, residual_list=[], max_v=256, residual_depth=1):
    ...
    cur_max_v = max_v // 4
    residual_list.append(decomse_diff / 255.0)
