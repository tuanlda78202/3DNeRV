...
def main():
    ...
    # dataset parameters
    parser.add_argument(
        "--vid", default=[None], type=int, nargs="+", help="video id list for training"
    )
    parser.add_argument(
        "--scale",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="augment frames between frames,  added to suffix!!!!",
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="1.25_80",
        help="base value/embed length for position encoding",
    )

    # FC + Conv parameters
    parser.add_argument(
        "--stem_dim_num", type=str, default="1024_1", help="hidden dimension and length"
    )
    parser.add_argument(
        "--fc_hw_dim", type=str, default="9_16_128", help="out size (h,w) for mlp"
    )
    parser.add_argument(
        "--expansion", type=float, default=8, help="channel expansion from fc to conv"
    )
    parser.add_argument("--reduction", type=int, default=2)
    parser.add_argument(
        "--strides", type=int, nargs="+", default=[5, 3, 2, 2, 2], help="strides list"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="lowest channel width for output feature maps",
    )
    parser.add_argument(
        "--single_res",
        action="store_true",
    ...
    parser.add_argument(
        "-j", "--workers", type=int, help="number of data loading workers", default=4
    )
    parser.add_argument(
        "-b", "--batchSize", type=int, default=1, help="input batch size"
    )
    parser.add_argument(
        "--not_resume_epoch",
        action="store_true",
        help="resuming start_epoch from checkpoint",
    )
    parser.add_argument(
        "-e", "--epochs", type=int, default=150, help="number of epochs to train for"
    )
    parser.add_argument(
        "--cycles", type=int, default=1, help="epoch cycles for training"
    )
    parser.add_argument(
        "--warmup",
        type=float,
    ...
    parser.add_argument(
        "--lr", type=float, default=0.001, help="learning rate, default=0.0002"
    )
    parser.add_argument(
        "--lr_type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="beta for adam. default=0.5,  added to suffix!!!!",
    )
    parser.add_argument(
        "--loss_type", type=str, default="L2", help="loss type, default=L2"
    )
    parser.add_argument(
        "--lw", type=float, default=1.0, help="loss weight,  added to suffix!!!!"
    )
    parser.add_argument(
        "--sigmoid", action="store_true", help="using sigmoid for output prediction"
    )

    # evaluation parameters
    parser.add_argument(
        "--eval_only", action="store_true", default=False, help="do evaluation only"
    )
    parser.add_argument(
        "--eval_freq",
        type=int,
        default=50,
        help="evaluation frequency,  added to suffix!!!!",
    )
    parser.add_argument(
        "--quant_bit", type=int, default=-1, help="bit length for model quantization"
    )
    parser.add_argument(
        "--quant_axis",
        type=int,
        default=0,
        help="quantization axis (-1 means per tensor)",
    )
    parser.add_argument(
        "--dump_images",
        action="store_true",
        default=False,
        help="dump the prediction images",
    )
    parser.add_argument(
        "--eval_fps",
        action="store_true",
        default=False,
        help="fwd multiple times to test the fps ",
    )
    ...
    parser.add_argument(
        "--debug", action="store_true", help="defbug status, earlier for train/eval"
    )
    parser.add_argument(
        "-p",
        "--print-freq",
    ...
    parser.add_argument(
        "--outf", default="unify", help="folder to output images and model checkpoints"
    )
    ...
    if args.prune_ratio < 1 and not args.eval_only:
        prune_str = "_Prune{}_{}".format(
            args.prune_ratio, ",".join([str(x) for x in args.prune_steps])
        )
    else:
        prune_str = ""
    extra_str = "_Strd{}_{}Res{}{}".format(
        ",".join([str(x) for x in args.strides]),
        "Sin" if args.single_res else f"_lw{args.lw}_multi",
        "_dist" if args.distributed else "",
        f"_eval" if args.eval_only else "",
    )
    norm_str = "" if args.norm == "none" else args.norm

    exp_id = (
        f"{args.dataset}/embed{args.embed}_{args.stem_dim_num}_fc_{args.fc_hw_dim}__exp{args.expansion}_reduce{args.reduction}_low{args.lower_width}_blk{args.num_blocks}_cycle{args.cycles}"
...
def train(local_rank, args):
    ...
    if prune_net:
        ...
        prune_base_ratio = args.prune_ratio ** (1.0 / len(args.prune_steps))
    ...
    if local_rank in [0, None]:
        ...
        writer = SummaryWriter(
            os.path.join(args.outf, f"param_{total_params}M", "tensorboard")
        )
    else:
        writer = None
    ...
    if os.path.isfile(checkpoint_path):
        ...
        if prune_net:
            ...
            print(
                f"Model sparsity at Epoch{args.start_epoch}: {sparisity_num / 1e6 / total_params}"
            )
        model.load_state_dict(checkpoint["state_dict"])
        print(
            "=> Auto resume loaded checkpoint '{}' (epoch {})".format(
    ...
    for epoch in range(args.start_epoch, total_epochs):
        ...
        if prune_net and epoch in args.prune_steps:
            ...
            for param in param_list:
                sparisity_num += (param.weight == 0).sum()
            print(
                f"Model sparsity at Epoch{epoch}: {sparisity_num / 1e6 / total_params}"
            )
        ...
        for i, (data, norm_idx) in enumerate(train_dataloader):
            ...
            if local_rank is not None:
                ...
                data, embed_input = data.cuda(non_blocking=True), embed_input.cuda(
                    non_blocking=True
                )
            # forward and backward
            output_list = model(embed_input)
            target_list = [
                F.adaptive_avg_pool2d(data, x.shape[-2:]) for x in output_list
            ]
            loss_list = [
                loss_fn(output, target, args)
                for output, target in zip(output_list, target_list)
            ]
            loss_list = [
                loss_list[i] * (args.lw if i < len(loss_list) - 1 else 1)
            ...
            if i % args.print_freq == 0 or i == len(train_dataloader) - 1:
                ...
                print_str = "[{}] Rank:{}, Epoch[{}/{}], Step [{}/{}], lr:{:.2e} PSNR: {}, MSSSIM: {}".format(
                    ...
                    RoundTensor(train_msssim, 4, False),
                )
        ...
        if local_rank in [0, None]:
            ...
            train_best_psnr = (
                train_psnr[-1] if train_psnr[-1] > train_best_psnr else train_best_psnr
            )
            train_best_msssim = (
                train_msssim[-1]
                if train_msssim[-1] > train_best_msssim
                else train_best_msssim
            )
            writer.add_scalar(
                f"Train/PSNR_{h}X{w}_gap{args.frame_gap}",
                train_psnr[-1].item(),
                epoch + 1,
            )
            writer.add_scalar(
                f"Train/MSSSIM_{h}X{w}_gap{args.frame_gap}",
                train_msssim[-1].item(),
                epoch + 1,
            )
            writer.add_scalar(
                f"Train/best_PSNR_{h}X{w}_gap{args.frame_gap}",
                train_best_psnr.item(),
                epoch + 1,
            )
            writer.add_scalar(
                f"Train/best_MSSSIM_{h}X{w}_gap{args.frame_gap}",
                train_best_msssim,
                epoch + 1,
            )
            print_str = (
                "\t{}p: current: {:.2f}\t best: {:.2f}\t msssim_best: {:.4f}\t".format(
                    h,
                    train_psnr[-1].item(),
                    train_best_psnr.item(),
            ...
            print(
                "Time/epoch: \tCurrent:{:.2f} \tAverage:{:.2f}".format(
                    ...
                    (epoch_end_time - start).total_seconds()
        ...
        if (epoch + 1) % args.eval_freq == 0 or epoch > total_epochs - 10:
            ...
            if local_rank in [0, None]:
                ...
                val_best_msssim = (
                    val_msssim[-1]
                    if val_msssim[-1] > val_best_msssim
                    else val_best_msssim
                )
                writer.add_scalar(
                    f"Val/PSNR_{h}X{w}_gap{args.test_gap}", val_psnr[-1], epoch + 1
                )
                writer.add_scalar(
                    f"Val/MSSSIM_{h}X{w}_gap{args.test_gap}", val_msssim[-1], epoch + 1
                )
                writer.add_scalar(
                    f"Val/best_PSNR_{h}X{w}_gap{args.test_gap}",
                    val_best_psnr,
                    epoch + 1,
                )
                writer.add_scalar(
                    f"Val/best_MSSSIM_{h}X{w}_gap{args.test_gap}",
                    val_best_msssim,
                    epoch + 1,
                )
                ...
                if is_val_best:
                    torch.save(
                        save_checkpoint, "{}/model_val_best.pth".format(args.outf)
                    )
...
def evaluate(model, val_dataloader, pe, local_rank, args):
    ...
    if args.quant_bit != -1:
        ...
        for k, v in cur_ckt.items():
            ...
            quant_v, new_v = quantize_per_tensor(
                v, args.quant_bit, args.quant_axis if large_tf else -1
            )
    ...
    for i, (data, norm_idx) in enumerate(val_dataloader):
        ...
        if args.dump_images:
            for batch_ind in range(args.batchSize):
                ...
                save_image(
                    output_list[-1][batch_ind], f"{visual_dir}/pred_{full_ind}.png"
                )
        ...
        if i % args.print_freq == 0:
            ...
            print_str = "Rank:{}, Step [{}/{}], PSNR: {}, MSSSIM: {} FPS: {}".format(
                ...
                i + 1,
                len(val_dataloader),
                RoundTensor(val_psnr, 2, False),
                RoundTensor(val_msssim, 4, False),
                round(fps, 2),
            )
            print(print_str)
            if local_rank in [0, None]:
