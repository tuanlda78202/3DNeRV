...
def main():
    ...
    parser.add_argument(
        "--shuffle_data", action="store_true", help="randomly shuffle the frame idx"
    )
    parser.add_argument(
        "--data_split",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="640_1280",
        help="video crop size",
    )
    parser.add_argument(
        "--resize_list",
        type=str,
        default="-1",
        help="video resize size",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="empty string for HNeRV, and base value/embed_length for NeRV position encoding",
    )
    parser.add_argument(
        "--ks", type=str, default="0_3_3", help="kernel size for encoder and decoder"
    )
    parser.add_argument(
        "--enc_strds", type=int, nargs="+", default=[], help="stride list for encoder"
    )
    parser.add_argument(
        "--enc_dim",
        type=str,
        default="64_16",
        help="enc latent dim and embedding ratio",
    )
    parser.add_argument(
        "--modelsize",
        type=float,
        default=1.5,
        help="model parameters size: model size + embedding parameters",
    )
    parser.add_argument(
        "--saturate_stages",
        type=int,
        default=-1,
        help="saturate stages for model size computation",
    )

    # Decoding parameters: FC + Conv
    parser.add_argument(
        "--fc_hw", type=str, default="9_16", help="out size (h,w) for mlp"
    )
    parser.add_argument(
        "--reduce", type=float, default=1.2, help="chanel reduction for next stage"
    )
    parser.add_argument(
        "--lower_width",
        type=int,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[5, 3, 2, 2, 2],
        help="strides list for decoder",
    )
    parser.add_argument(
        "--num_blks",
        type=str,
        default="1_1",
        help="block number for encoder and decoder",
    )
    parser.add_argument(
        "--conv_type",
        default=["convnext", "pshuffel"],
        type=str,
        nargs="+",
        help="conv type for encoder/decoder",
        choices=["pshuffel", "conv", "convnext", "interpolate"],
    )
    parser.add_argument(
        "--norm",
        default="none",
    ...
    parser.add_argument(
        "--not_resume", action="store_true", help="not resume from latest checkpoint"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="1_1",
        help="residual blocks and percentile to save",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--loss", type=str, default="Fusion6", help="loss type, default=L2"
    )
    parser.add_argument(
        "--out_bias",
        default="tanh",
        type=str,
        help="using sigmoid/tanh/0.5 for output prediction",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="bit length for model quantization",
    )
    parser.add_argument(
        "--quant_embed_bit",
        type=int,
        default=6,
        help="bit length for embedding quantization",
    )
    parser.add_argument(
        "--quant_axis",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="concat the prediction images into video",
    )
    parser.add_argument(
        "--eval_fps",
        action="store_true",
    ...
    parser.add_argument(
        "--encoder_file", default="", type=str, help="specify the embedding file"
    )
    ...
    args.enc_strd_str, args.dec_strd_str = ",".join(
        [str(x) for x in args.enc_strds]
    ), ",".join([str(x) for x in args.dec_strds])
    extra_str = "Size{}_ENC_{}_{}_DEC_{}_{}_{}{}{}".format(
        ...
        "_shuffle_data" if args.shuffle_data else "",
    )
    args.quant_str = f"quant_M{args.quant_model_bit}_E{args.quant_embed_bit}"
    embed_str = f"{args.embed}_Dim{args.enc_dim}"
    exp_id = (
        f"{args.vid}/{args.data_split}_{embed_str}_FC{args.fc_hw}_KS{args.ks}_RED{args.reduce}_low{args.lower_width}_blk{args.num_blks}"
...
def train(local_rank, args):
    ...
    if "pe" in args.embed or "le" in args.embed:
        ...
        embed_dim = (
            int(embed_ratio * args.modelsize * 1e6 / args.full_data_length / embed_hw)
            if embed_ratio < 1
    ...
    fix_ch_stages = (
        len(args.dec_strds) if args.saturate_stages == -1 else args.saturate_stages
    )
    a = ch_reduce * sum(
        [
    ...
    if local_rank in [0, None]:
        encoder_param = (
            sum([p.data.nelement() for p in model.encoder.parameters()]) / 1e6
        )
        decoder_param = (
            sum([p.data.nelement() for p in model.decoder.parameters()]) / 1e6
        )
        total_param = decoder_param + embed_param / 1e6
        args.encoder_param, args.decoder_param, args.total_param = (
            encoder_param,
            decoder_param,
            total_param,
        )
    ...
    if args.eval_only:
        ...
        results_list, hw = evaluate(
            model, full_dataloader, local_rank, args, args.dump_vis, huffman_coding=True
        )
        ...
        for i, (metric_name, best_metric_value, metric_value) in enumerate(
            ...
            best_metric_value = (
                best_metric_value
                if best_metric_value > metric_value.max()
                else metric_value.max()
            )
        ...
        if local_rank in [0, None]:
            ...
            Dump2CSV(
                args, best_metric_list, results_list, [torch.tensor(0)], "eval.csv"
            )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, sample in enumerate(train_dataloader):
            ...
            final_loss = loss_fn(
                img_out * inpaint_mask, img_gt * inpaint_mask, args.loss
            )
            optimizer.zero_grad()
            final_loss.backward()
            optimizer.step()
            ...
            if local_rank in [0, None]:
                ...
                for i, (metric_name, best_metric_value, metric_value) in enumerate(
                    ...
                    if "psnr" in metric_name:
                        writer.add_scalar(
                            f"Val/{metric_name}_{hw}", metric_value.max(), epoch + 1
                        )
                        writer.add_scalar(
                            f"Val/best_{metric_name}_{hw}", best_metric_value, epoch + 1
                        )
...
def Dump2CSV(args, best_results_list, results_list, psnr_list, filename="results.csv"):
    ...
    result_dict.update(
        {
            f"best_{k}": RoundTensor(v, 4 if "ssim" in k else 2)
            for k, v in zip(args.metric_names, best_results_list)
        }
    )
    result_dict.update(
        {
            f"{k}": RoundTensor(v, 4 if "ssim" in k else 2)
            for k, v in zip(args.metric_names, results_list)
            if "pred" in k
        }
    )
